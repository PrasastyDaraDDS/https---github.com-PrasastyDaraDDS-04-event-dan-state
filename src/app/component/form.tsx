...
export default function Form() {
    ...
    const [error, setError] = useState(null); // Menghapus tanda kutip
    const [status, setStatus] = useState('typing');

    if (status === 'success') { // Mengganti 'status' menjadi 'jstatus'
        return <h1>Yayyy.. Jawaban Benar!!</h1>;
    }

    async function handleSubmit(e) { // Menghapus tipe data TypeScript
        e.preventDefault();
    ...
    return (
        <div className="w-full max-w-xs">
            ...
            <form
                ...
                {error !== null && <p className="Error text-red-500 text-sm">{error.message}</p>} {/* Menggunakan properti 'message' */}
            </form>
        </div>
    );
}
...
export function Form_2(){
    ...
    const [lastName, setLastName] = useState(''); // Menghapus tanda kutip
